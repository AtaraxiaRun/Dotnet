...
namespace AspNetCore.IOC
{
    internal class Program
    {
        ...
        /// 5.依赖注入的逻辑其实简单，比如 ServiceDescription 保存每一个被注入的实例,，IServiceCollection 保存实例的集合，IServiceProvider 管理容器的获取，释放   IServiceScope 里面有一个IServiceProvider对象，代表的是子容器，AutoFac就是支持注入的方式比较多，比如程序集注入，配置注入，属性注入，其实推荐用Scrutor，对Net Core依赖注入的方式进行扩展了很多多样化的注册函数
        /// 6.看源码调试可以学习到很多东西
        /// </summary>
